...
export const useSearch = (data, fields, minLengthToShow = 1) => {
  ...
  // const filteredData = data.filter((item) => fields.some((field) => item[field].toLowerCase().includes(searchTerm.toLowerCase())));
  const filteredData = data.filter((item) =>
    fields.some((field) => getNestedValue(item, field)?.toString().toLowerCase().includes(searchTerm.toLowerCase()))
  );
